...
let backgroundColor = '';
let lettersColor = '';
let lettersColorEnlighted = '';
let fontSize = 0;
const defineLetters = (lettersSequence = '!@#$%¨&*()_+-=/?123456789ABCDEFGHIJKLMNOPQRSTUVWYXZ') => {
    letters = lettersSequence;
};
const defineLettersColor = (color = '#00FF00') => {
    const { r, g, b } = hexToRgb(color);
    const { re, ge, be } = rgbEnlighter({ r, g, b });
    lettersColor = `${r}, ${g}, ${b}`;
    lettersColorEnlighted = `${re}, ${ge}, ${be}`;
};
const defineBackgroundColor = (color = '#000000') => {
    backgroundColor = color;
};
const defineFontSize = (size = 12) => {
    fontSize = size;
};
defineLetters();
...
export { defineLetters, defineLettersColor, defineBackgroundColor, defineFontSize };
